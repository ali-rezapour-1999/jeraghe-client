...
export function DashboardSidebar() {
    ...
    <SidebarMenuItem key={path}>
      ...
      >
        {path === "#logout" ? (
          <Button
            onClick={logout}
            className={`flex items-center dark:hover:bg-primary/20 hover:bg-red-800/20  h-12 ${!isOpen ? "justify-center" : "justify-start"}`}
            variant="ghost"
          >
          ...
          <Link
            href={path}
            className={`flex items-center dark:hover:bg-primary/20 hover:bg-primary-dark/10  h-12 ${!isOpen ? "justify-center" : "justify-start"}`}
            variant="ghost"
          >
  ...
  return (
    <Sidebar
      side="right"
      className={cn(
        `fixed top-[54px] right-0 z-40 h-[calc(100vh-44px)] dark:bg-primary-dark/30 bg-primary text-sidebar-foreground transition-all duration-300 ease-in-out md:sticky md:top-[64px] md:h-[calc(100vh-64px)] md:w-64 lg:w-72 !px-3 md:px-0 ${isOpen ? "w-[200px] md:w-64 lg:w-72" : "w-[100px] md:w-16 lg:w-20"}`
      )}
    >
      ...
      <SidebarRail
        ...
        className={`dark:bg-primary-dark/70 bg-primary hover:bg-white flex items-center rounded-l-full w-5`}
      >
        {isOpen ? (
